...
__global__ void Mul_Naive(float *A, float *B, float *C, int rA, int common, int lB) 
{
   ...
   if(row < rA && col < lB) 
   {
       ...
       {
          value += A[common * row + k] * B[lB * k + col];
       }
       C[lB * row + col] = value;
   }
}
...
mat_mul_naive = mod.get_function("Mul_Naive")
...
for r in xrange(1,300,1):
  ...
  start = time.time()
  mat_mul_naive(a_gpu, b_gpu, c_gpu, np.int32(MATRIX_A_ROWS) , np.int32(MATRIX_A_COLS) ,np.int32(MATRIX_B_COLS), grid = ((MATRIX_B_COLS - 1) // BLOCK_SIZE  + 1, (MATRIX_A_ROWS - 1) // BLOCK_SIZE  + 1, 1) , block = (BLOCK_SIZE , BLOCK_SIZE , 1))
...
naive_cu_times_blocksize = np.zeros_like(x_block).astype(np.float64)
...
for s in xrange(0, len(tile_size_array)):
  ...
  iterative = 50
  temp_naive = np.zeros_like(np.arange(0, iterative, 1)).astype(np.float64)
  temp_tile = np.zeros_like(np.arange(0, iterative, 1)).astype(np.float64)

  for j in xrange(0, iterative):
    start = time.time()
    mat_mul_naive(a_gpu_block, b_gpu_block, c_gpu_block, np.int32(MATRIX_A_ROWS) , np.int32(MATRIX_A_COLS) ,np.int32(MATRIX_B_COLS), grid = ((MATRIX_B_COLS - 1) // BLOCK_SIZE  + 1, (MATRIX_A_ROWS - 1) // BLOCK_SIZE  + 1, 1) , block = (BLOCK_SIZE , BLOCK_SIZE , 1))
    temp_naive[j] = time.time() - start
  ...
  naive_cu_times_blocksize[s] = np.mean(temp_naive)
...
if MAKE_PLOT:
  ...
  plt.plot(x, naive_cu_times,'g')
  plt.plot(x, tile_cu_times,'b')
  plt.legend(['python mul', 'gpu naive mult', 'gpu tile mult'], loc='upper left')
  ...
  plt.plot(x, naive_cu_times,'g')
  plt.plot(x, tile_cu_times,'b')
  plt.legend(['gpu naive mult', 'gpu tile mult'], loc='upper left')
  plt.xlabel('matrix ratio increase factor')
  plt.ylabel('only gpu times')
  plt.gca().set_xlim((min(x), max(x)))
  
  plt.subplot(313)
  plt.plot(np.array(tile_size_array).astype(np.int32), naive_cu_times_blocksize,'g')
